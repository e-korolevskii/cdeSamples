...
{
    public class eCDMyTestPluginDeviceTypes : TheDeviceTypeEnum
    {
    ...
    [EngineAssetInfo(
        ...
        EngineID = "{11de2724-a038-4c2b-9f26-cd41005548cf}",
        IsService = true,
    ...
    class cdePluginService1 : ThePluginBase
    {
        ...
        public override bool CreateUX()
        {
            if (!mIsUXInitCalled)
            {
                ...
                var tFlds = TheNMIEngine.CreateEngineForms(MyBaseThing, TheThing.GetSafeThingGuid(MyBaseThing, "MYNAME"), "List of cdePluginService1", null, 20, 0x0F, 0xF0, TheNMIEngine.GetNodeForCategory(), "REFFRESHME", true, new eCDMyTestPluginDeviceTypes(), eCDMyTestPluginDeviceTypes.cdeThingDeviceTypeA);
                TheFormInfo tForm = tFlds["Form"] as TheFormInfo;
                tForm.AddButtonText = "Add new cdePluginService1";
            ...
            foreach (TheThing tDev in tDevList)
            {
                if (!tDev.HasLiveObject)
                {
                    ...
                    switch (tDev.DeviceType)
                    {
                        ...
                        //case eCDMyTestPluginDeviceTypes.cdeThingDeviceTypeA:
                        //    TheThingRegistry.RegisterThing(new cdeThingDeviceTypeA(tDev, this));
                        //    break;
                        default:
                            // Assume the eCDMyTestPluginDeviceTypes field names match the class names: find the field that corresponds to the TheThing.DeviceType being requested
                            var fields = typeof(eCDMyTestPluginDeviceTypes).GetFields();
                            foreach (FieldInfo deviceTypeField in fields)
                            {
                                var deviceTypeValue = deviceTypeField.GetValue(new eCDMyTestPluginDeviceTypes()) as string;
                                ...
                                if (deviceTypeValue == tDev.DeviceType)
                                {
                                    ...
                                    var deviceTypeClass = Type.GetType("CDMyTestPlugin." + deviceTypeField.Name);
                                    if (deviceTypeClass == null)
                                    {
                                        deviceTypeClass = Type.GetType("CDMyTestPlugin.ViewModel." + deviceTypeField.Name);
                                    }
                                    if (deviceTypeClass != null)
                                    {
